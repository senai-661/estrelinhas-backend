...
class Aluno {
    ...
    // listarAlunos — só 1 tabela, continua direto
    static async listarAlunos(): Promise<Array<Aluno> | null> {
        try {
            const lista: Array<Aluno> = [];
            const query = `SELECT id_aluno, nome, sobrenome, cpf, data_nascimento, endereco, email, celular, status_aluno FROM Aluno ORDER BY nome;`;
    ...
    // listarAluno por ID — só 1 tabela, continua direto
    ...
    // listarAlunosAtivos — usa a VIEW (envolve aluno + matricula + plano)
